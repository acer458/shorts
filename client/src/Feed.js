...
if (!window.__FEED_ROOT_STYLES__) {
  ...
  rootStyle.innerHTML = `
    ...
    * {
    ...
    .feed-btn-anim:active, .feed-btn-anim:focus { transform: scale(.97); box-shadow: 0 2px 6px #0001; }
  `;
  ...
  window.addEventListener("keydown", e => {
    ...
    if ((e.metaKey && e.altKey && (e.key === "I" || e.key === "J"))) { e.preventDefault(); return false; }
...
function HeartSVG({ filled, size = 25, color }) {
  return (
    <svg aria-label={filled ? "Unlike" : "Like"} height={size} width={size} viewBox="0 0 48 48">
      <path
        fill={filled ? (color || "#ed4956") : "none"}
        stroke={filled ? (color || "#ed4956") : "#bbb"}
        strokeWidth="3"
  ...
  return (
    <div className="overlay" style={{
      ...
      <div className="continue-card"
        ...
        <button className="continue-btn feed-btn-anim"
          style={{
...
// For comment likes/persisted reply
...
export default function Feed() {
  ...
  // -- Continue overlay: [{video: count, ...}], blockPlayback:{fname:true}
  const [playCounts, setPlayCounts] = useState({});
  const [blockPlayback, setBlockPlayback] = useState({});   // filename => true

  function handleContinue(filename) {
    setPlayCounts(pc => {
      const clone = { ...pc }; clone[filename] = 0; return clone;
    });
    setBlockPlayback(bp => {
      const clone = { ...bp }; delete clone[filename]; return clone;
    });
    // Resume video
    let vref = null;
  ...
  // Count uninterrupted plays; resets on seeking or user interaction. 3 => block.
  ...
  function handleTimeUpdate(idx, filename, e) {
    ...
    if (vid?.duration && vid.duration > 3 && vid.currentTime !== undefined) {
      // When video ends, increment play count (once per play)
  ...
  function handleSeek(idx, e, isTouch = false) {
    ...
    if (vid && vid.duration && isFinite(vid.duration)) {
      vid.currentTime = Math.max(0, Math.min(percent, 1)) * vid.duration; // triggers onSeeked
    }
  }
  ...
  // For reply action per comment, controlled
  ...
  // force rerender on comment like change (since persisted in LS)
  const [tick, setTick] = useState(0);
  function forceRerender() { setTick(t => t+1); }

  // comment rendering (only in modal)
    ...
    return (comments || []).map((c, i) => {
      ...
      return (
        <div key={i} className="comment-item"
          style={{
            display: 'flex', alignItems: 'flex-start', marginBottom: 18, fontFamily: "inherit",
            borderRadius: 10, position:"relative", paddingBottom: replies.length > 0 ? 4 : 0, background:"none"
          }}>
          <img
            src={c.avatar || fakeAvatar(i)}
            style={{ width: 31, height: 31, borderRadius: "50%", marginRight: 13, marginTop: 2 }}
            alt="avatar"
          />
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{display:'flex',alignItems:'flex-start',gap:9}}>
              ...
              <button
                ...
                style={{
                  background: "none", border: "none", color: liked ? "#ed4956" : "#bbb",
                  outline: "none",
                  marginLeft: 13, display: "flex", alignItems: "center", minWidth:30, padding:0,
                  fontWeight: 500, borderRadius: 7, cursor: "pointer", fontSize: "16px",
              ...
              >
                <HeartSVG filled={liked} size={18} color={liked && "#ed4956"} />
                <span style={{
            ...
            <div style={{ display: 'flex', alignItems:'center',
              fontSize: 12, color:'#a8a8a8', gap: 13, marginTop: 1 }}>
              <span>{c.time || fakeTime(i)}</span>
              <button
                className="feed-btn-anim"
                style={{
                  cursor: "pointer", color:"#23aae6", fontWeight: 600,
                  border:"none", background:"none", borderRadius:6, padding: "1.5px 10px 1.5px 8px", fontSize: 12,
            ...
            {replies.length ? (
              <div className="comment-replies" style={{
                marginLeft: 9, marginTop: 9, paddingLeft: 10, borderLeft: "1.5px solid #202636", fontSize: 14, background:"none"
              }}>
                {replies.map((r, j) =>
                  <div key={j} style={{marginBottom:8,display:"flex",alignItems:'flex-start',gap:9}}>
            ...
            {showActiveReply && activeReply[0] === filename && activeReply[1] === i && (
              <div style={{marginTop:10, marginLeft:0}}>
                <input
                  ...
                  style={{
                    width: "78%", background: "#23263a", border: "none",
                    borderRadius: 20, padding: "7px 15px", color: "#fff", fontSize: 14,
                    marginRight: 7, outline:"none"
                  }}
                ...
                <button
                  className="feed-btn-anim"
                  style={{
                    color: "#22d1ff", fontWeight: 600, fontSize: 14,
                    background: "none", border: "none", borderRadius: 6,
                    cursor: (commentInputs[filename+":"+i]||"").trim() !== "" ? "pointer" : "not-allowed",
                    opacity: (commentInputs[filename+":"+i]||"").trim() !== "" ? 1 : 0.55,
                    marginRight:5
                  }}
                ...
                <button onClick={() => setActiveReply([null,null])}
                   className="feed-btn-anim"
                  style={{
                    background:'none', border:'none', color:"#a1b3c6",fontSize:12,cursor:'pointer',borderRadius:6
                  }}>Cancel</button>
              </div>
            )}
          </div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            padding: "9px 32px", cursor: "pointer", fontFamily:'inherit'
          }}>
          Back to Feed
        </button>
      </div>
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: "#222f", color: "#fff", fontWeight: 600, fontSize: 16,
            padding: "6px 19px", borderRadius: 16, border: "none", cursor: "pointer", fontFamily:'inherit'
          }}>
          ← Feed
        </button>
        <video
        ...
        <div style={{
          ...
          <div style={{
            width: `${Math.min(prog * 100, 100)}%`, height: "100%",
            background: "rgb(42, 131, 254)", transition: "width 0.22s cubic-bezier(.4,1,.5,1)", pointerEvents: "none"
          }} />
        </div>
        <div style={{
        ...
        <div style={{
          ...
          {/* No first comment below caption! */}
          <div
        ...
        {showComments === filename &&
          <div style={{
            ...
            justifyContent:"flex-end", fontFamily:"inherit"
          }}
            onClick={() => setShowComments(null)}
          >
            <div className="comments-modal"
              style={{
                backgroundColor: "#14151d", borderTopLeftRadius: 13, borderTopRightRadius: 13, padding: 15,
                minHeight: '36vh', height: '70vh', display: 'flex', flexDirection: 'column', maxWidth: 500,
            ...
            >
              ...
              <div style={{
                ...
                paddingTop: 9, borderTop: '1px solid #262626', fontFamily:"inherit"
              }}>
                <input
                  type="text"
                  placeholder="Add a comment…"
                  style={{
                    flex: 1, backgroundColor: "#23263a", border: "none",
                    borderRadius: 20, padding: "10px 15px", color: "white", fontSize: 14
                  }}
                  value={commentInputs[filename] || ""}
                  onChange={e => setCommentInputs(prev => ({...prev, [filename]: e.target.value }))}
                  onKeyDown={e => e.key === "Enter" && (commentInputs[filename]||"").trim() !== "" && (()=>handleAddComment(0, filename))}
                />
                <button className="feed-btn-anim"
                  style={{
                    ...
                    opacity: (commentInputs[filename]||"").trim()!==""?1:0.55
                  }}
  ...
  // ---------- FEED MODE (ALL VIDEOS) --------------
  if (!loading && shorts.length === 0) {
    return (
      <div style={{ color: "#bbb", textAlign: "center", marginTop: 120, fontSize: 20, fontFamily: "inherit" }}>
              ...
              {caption && (
                <div style={{ display:"flex", alignItems:"flex-end", minHeight:"26px", maxWidth: 500 }}>
                  <div style={{
                    ...
                    maxHeight:showFull?"none":"2.8em", overflow:showFull?"visible":"hidden", textOverflow:"ellipsis",
                    display:"-webkit-box", WebkitLineClamp:showFull?"unset":2, WebkitBoxOrient:"vertical",
            ...
            {showComments === filename &&
              ...
              >
                ...
                >
                  ...
                  <div style={{
                    display:'flex',alignItems:'center',paddingTop:9,borderTop:'1px solid #262626',fontFamily:"inherit"
                  }}>
                    <input
                      type="text"
                      placeholder="Add a comment…"
